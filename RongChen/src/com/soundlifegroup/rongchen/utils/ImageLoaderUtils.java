...
public class ImageLoaderUtils {
	...
	private static void config() {
		...
		// .showImageOnLoading(R.drawable.icon_normal) // 设置图片在下载期间显示的图片
				.showImageForEmptyUri(R.drawable.icon_normal) // 设置图片Uri为空或是错误的时候显示的图片
				.showImageOnFail(R.drawable.icon_normal) // 设置图片加载或解码过程中发生错误显示的图片
				...
				.imageScaleType(ImageScaleType.IN_SAMPLE_INT) // 设置图片以如何的编码方式显示,EXACTLY_STRETCHED:图片会缩放到目标大小完全
																// IN_SAMPLE_INT图像将被二次采样的整数倍
				.build();
	}
	...
	public static void initImageLoader(Context context) {
		...
		ImageLoaderConfiguration config = new ImageLoaderConfiguration.Builder(
				...
				.memoryCacheSize(512 * 1024).discCacheSize(50 * 1024 * 1024)
				.imageDownloader(new BaseImageDownloader(context))
				.defaultDisplayImageOptions(DisplayImageOptions.createSimple())
				.tasksProcessingOrder(QueueProcessingType.LIFO)
				.diskCacheSize(50 * 1024 * 1024).diskCacheFileCount(100) // 缓存的文件数量
...
//
//	public void loadImage_one(String iamgeUrl, ImageView imageView) {
//		imageLoader.displayImage(iamgeUrl, imageView);
//	}
	...
	public void loadCacheImage(String imageUrl, ImageView imageView) {
		DisplayImageOptions options = new DisplayImageOptions.Builder()
				...
				.showImageOnFail(R.drawable.icon_normal)
